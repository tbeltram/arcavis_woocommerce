...
class WooCommerce_Arcavis_Create_Products_Settings{
	...
	public function create_products_init(){		
		...
		if($lastSync == ''){
			...
			$url = $options['arcavis_link'].'/api/articles?mainArticleId=0&includeStock=true&page='.$lastSyncPage.'&pageSize=25';
			$request_args = array(
			  'headers' => array(
				...
				foreach ($products->Result as $product) {
					...
					if(array_key_exists("HasVariations", $product)){
						if($product->HasVariations == 'true'){
							...
							$variation_url = $options['arcavis_link'].'/api/articles?mainArticleId='.$product->Id.'&includeStock=true';
	...
	public function update_products(){
		...
		$lastSync = $wc_arcavis_shop->get_last_sync('articles');
		if($lastSync != ''){	
			$url = $options['arcavis_link'].'/api/articles?mainArticleId=0&changedSinceTicks='.$lastSync;
			...
			if(isset($products->DeletedIds) && !empty($products->DeletedIds)){
				...
				if(!empty($products_by_article_id)){
					...
					//$wpdb->query("DELETE  FROM ".$wpdb->prefix."terms a INNER JOIN ".$wpdb->prefix."term_taxonomy b ON a.term_id = b.term_id WHERE b.count = 0 b.taxonomy='product_cat'");
				}
			...
			if(!empty($products->Result)){
				foreach ($products->Result as $product) {
					...
					$this->update_article_stock_by_id($options,$product->Id);
					if(array_key_exists("HasVariations", $product)){
						if($product->HasVariations == 'true'){
							wp_set_object_terms($post_id, 'variable', 'product_type');
							$variation_url = $options['arcavis_link'].'/api/articles?mainArticleId='.$product->Id.'&includeStock=true';
	...
	##This function is used to update the articles stock
